var mBoardPagination = angular.module('mBoardPagination', ['mGlobalService']);

//-----------------------------------------------------------------------------
mBoardPagination.controller('myPaginationController', ['$scope', '$location','$attrs', '$parse','$http', 'myHttpService', 'mGlobalServiceData',
    function ($scope, $location, $attrs, $parse,  $http, myHttpService, mGlobalServiceData) {
        var pageInfo = mGlobalServiceData.pageInfo;
            ...
            $http.get($scope.countAllApiUrl )
                .success(function(totalCount) {
                    ...
                    */
                    ...
                    var nIndexStart = ((mGlobalServiceData.pageInfo.currentPage-1) * mGlobalServiceData.pageInfo.listPerPage)+1 ;
                    //console.log( "nIndexStart ="+nIndexStart ); //debug
                    var listIndexAry=[];
                    for (i = 0; i < mGlobalServiceData.pageInfo.listPerPage; i++) {
                        listIndexAry.push ( nIndexStart+i );
                        //console.log( "push :"+(nIndexStart+i) ); //debug
                    }
                    angular.copy(listIndexAry, mGlobalServiceData.pageInfo.listIndexAry);
                })
                .error (function () {
...
mBoardPagination.directive('myPaginationDirective', function() {
    return {
        restrict: 'E',
        templateUrl: 'boardViews/pagination.html',
